chrome.storage.local.get(
    [
        ...
        option.keyPress == null && chrome.storage.local.set({ keyPress: ['ControlLeft', 'ShiftLeft'] });
        option.fullScreen == null && chrome.storage.local.set({ fullScreen: true });
        option.windowOut == null && chrome.storage.local.set({ windowOut: true });
        option.windowFocus == null && chrome.storage.local.set({ windowFocus: true });
        ...
        chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
            if (tab.url.match(/(https?:\/\/|http?:\/\/)*(whatsapp)\.(com)/g)) {
                if (changeInfo['status'] === "complete") {
                    ...
                    chrome.scripting.executeScript({ target: { tabId: tab.id, allFrames: true }, func: () => { window.tabId = tab.id } })
                }
            }
        });
    }
